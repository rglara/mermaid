...
import type { /*TreemapNode,*/ StorymapAst } from './types.js';
// import { buildHierarchy } from './utils.js';

/**
 * Populates the database with data from the Storymap AST
 * @param ast - The Storymap AST
 */
const populate = (ast: StorymapAst) => {
...
export const parser: ParserDefinition = {
  parse: async (text: string): Promise<void> => {
    try {
      ...
      const parseFunc = parse as (diagramType: string, text: string) => Promise<StorymapAst>;
      const ast = await parseFunc('storymap', text);
      log.debug('Storymap AST:', ast);
      populate(ast);
    } catch (error) {
      log.error('Error parsing storymap:', error);
      throw error;
    }
  },
};
